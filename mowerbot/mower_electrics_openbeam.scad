...
block_brace_w = 5;
module pillow_block() {
    block_outside = 2*bearing_or + 2 * wall_th;
    difference() {
        union() {
            translate([0, 0, wall_th/2])
                cube([4 * bearing_or, beam_w, wall_th], center=true);
            translate([0, 0, block_outside/4])
                cube([block_outside, beam_w, block_outside/2], center=true);
            translate([0, 0, bearing_or + wall_th])
                rotate([90, 0, 0]) 
                cylinder(r=block_outside/2, h=beam_w, center=true);
            translate([bearing_or + wall_th, 0, sqrt(2)/4*block_brace_w + wall_th/2])
                rotate([0, 45, 0])
        ...
        translate([0, 0, bearing_or + wall_th])
            rotate([90, 0, 0]) 
            cylinder(r=bearing_or, h=beam_w+1, center=true, $fn=64);
        translate([2*bearing_or - wall_th - beam_hole_or, 0, 0])
