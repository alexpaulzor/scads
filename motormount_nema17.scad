...
axle_distance = 41.6;
final_axle_distance = axle_distance + 58.6; //stepper_center / 2 + axle_bearing_od + final_bearing_od / 2 + 2 * bearing_gap;
final_axle_od = 12.7;
...
gear_ratio = motor_gear_teeth / axle_large_gear_teeth * axle_small_gear_teeth / final_gear_teeth;
...
module set_screw_collar(od, id, height) {
    ...
    nut_shaft_distance = ((od - id) / 2 - m3_nut_depth)  / 2;	// distance between inner face of nut and shaft, can be negative.
    ...
    difference()
	 {	 
		...
		cylinder(r=id/2,h=height,$fn=id*4);
        ...
        for(j=[1:no_of_nuts]) rotate([0,0,j*nut_angle])
			...
			{
				//entrance
				translate([0,-height/4-0.5,id/2+m3_nut_depth/2+nut_shaft_distance]) cube([m3_nut_flats,height/2+1,m3_nut_depth],center=true);
				...
				//grub screw hole
				rotate([0,0,22.5]) cylinder(r=m3_dia/2,h=od/2+1,$fn=30);
			}
...
module motor_gear() {
    //simple_gear(motor_shaft_length, motor_gear_pd / 2, motor_shaft_od /2);
    //spur_gear(motor_shaft_od, motor_gear_teeth, romeTeeth=0, pitch_ratio=1.5, gearHeight=gear_height);
    translate([0, 0, 0])
        set_screw_collar(stepper_center - 2, axle_od, plate_height);
...
module axle_large_gear() {
    //simple_gear(gear_height, axle_large_gear_pd / 2, axle_od / 2);
    //spur_gear(axle_od, axle_large_gear_teeth, rome=1, romeTeeth=axle_small_gear_teeth, gearHeight=gear_height);
    ...
    gear (circular_pitch=motor_pitch,
        ...
        bore_diameter=axle_od,
		rim_width = 2);
    translate([0, 0, gear_height]) 
        axle_small_gear();
//    translate([0, 0, 2 * gear_height])
        set_screw_collar(axle_small_gear_pd, axle_od, gear_height);
}
module axle_small_gear() {
    //simple_gear(gear_height, axle_small_gear_pd / 2, axle_od / 2);
//    spur_gear(axle_od, small_gear_teeth);
...
module final_gear() {
    //simple_gear(gear_height, final_gear_pd / 2, final_axle_od / 2);
    //spur_gear(final_axle_od, final_gear_teeth, gearHeight=gear_height);
    gear (circular_pitch=final_pitch,
		gear_thickness = gear_height,
		rim_thickness = gear_height,
		hub_thickness = gear_height,
        ...
        bore_diameter=final_axle_od,
		rim_width = 2);
    translate([0, 0, gear_height]) 
        set_screw_collar(3 * final_axle_od, final_axle_od, plate_height);
}
...
module model_gears() {
    ...
    # translate([stepper_size / 2, stepper_size / 2, 0])
        ...
        motor_gear();
    translate([stepper_size / 2, stepper_size / 2 + axle_distance, plate_height]) 
        axle_large_gear();
    //translate([stepper_size / 2, stepper_size / 2 + axle_distance, plate_height + gear_height]) 
    //    axle_small_gear();
    translate([stepper_size / 2, stepper_size / 2 + final_axle_distance, plate_height + gear_height]) 
        rotate([0, 0, 180 / final_gear_teeth])
        final_gear();
}
...
module placeholder_pulleys() {
    # translate([stepper_size/2, stepper_size/2+axle_distance, plate_height + plate_gap - pulley_height])
    axle_small_pulley();
    translate([stepper_size/2, stepper_size/2+axle_distance, plate_height])
        axle_large_pulley();
    ...
    # translate([stepper_size/2, stepper_size/2+axle_distance + final_axle_distance, plate_height + plate_gap - pulley_height]) 
        final_pulley();
}
...
module bearing_plate() {
    difference() {
        cube([stepper_size, length, plate_height / 2]);
        translate([stepper_size/2, stepper_size/2+axle_distance,
                (plate_height - axle_bearing_height) / 2]) 
            cylinder(plate_height, axle_bearing_od/2, axle_bearing_od/2);
        ...
        translate([stepper_size/2, stepper_size/2 + final_axle_distance,
            ...
            cylinder(plate_height, final_bearing_od/2, final_bearing_od/2);
...
module full_plate() {
     half_plate();
     translate([0, 0, plate_height]) mirror([0, 0, 1]) half_plate();
...
module plate_end() {
    difference() {
        ...
        translate([spacer_width + hole_offset, 0, plate_height + hole_offset])
            rotate([-90, 0, 0])
                cylinder(end_plate_height, hole_ID/2, hole_ID/2);
        translate([stepper_size - spacer_width - hole_offset, 0, plate_height + hole_offset])
            rotate([-90, 0, 0])
                cylinder(end_plate_height, hole_ID/2, hole_ID/2);
        translate([spacer_width + hole_offset, 0, plate_gap - hole_offset - plate_height])
            rotate([-90, 0, 0])
                cylinder(end_plate_height, hole_ID/2, hole_ID/2);
        translate([stepper_size - spacer_width - hole_offset, 0, plate_gap - hole_offset - plate_height])
            rotate([-90, 0, 0])
                cylinder(end_plate_height, hole_ID/2, hole_ID/2);
    }
...
module model() {
    ...
    //translate([0, length + end_plate_height, plate_height])
        //mirror([0, 1, 0]) plate_end();
    
    # translate([0, 0, -stepper_length]) motor();
    
    * translate([0, stepper_size - spacer_width, plate_height]) spacer();
    * translate([stepper_size - spacer_width, stepper_size - spacer_width, plate_height]) spacer();
...
module layout() { 
    translate([0, 0, 0])
        rotate([0, 0, 0])
            motor_gear();
    translate([axle_large_gear_pd / 1.5, -axle_large_gear_pd / 2, 0])
        rotate([0, 0, 0])
            axle_large_gear();
    translate([0, final_gear_pd/2 + motor_gear_pd / 2 + 5, 0])
        rotate([0, 0, 0])
            final_gear();
}
